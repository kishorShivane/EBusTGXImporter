...
namespace EBusTGXImporter.Core.Helpers
{
    public class Helper
    {
        ...
        public void MoveSuccessStatusFile(string currentpath, string dbname)
        {
            try
            {
                ...
                string dirPath1 = Constants.DirectoryPath + @"\" + dbname + @"\" + @"Out\" + DateTime.Now.Year + @"\" + DateTime.Now.ToString("MMMM") + @"\" + DateTime.Now.Day + @"\Status";
                string path2 = dirPath1 + @"\" + file;
        ...
        public void MoveErrorStatusFile(string currentpath, string dbname)
        {
            try
            {
                ...
                string file = Path.GetFileName(currentpath);
                //Current Path
                string dirPath1 = Constants.DirectoryPath + @"\" + dbname + @"\" + @"Error\" + DateTime.Now.Year + @"\" + DateTime.Now.ToString("MMMM") + @"\" + DateTime.Now.Day + @"\Status";
                //New Path
                string path2 = dirPath1 + @"\" + file;
        ...
        public int GetTripBalanceFromProductData(string productData)
        {
            try
            {
                ...
                {
                    char[] charArrayProdData = productData.ToCharArray();
                    string tripBalance = charArrayProdData[2].ToString() + charArrayProdData[3].ToString();
        ...
        public int GetTripRechargedFromProductData(string productData)
        {
            try
            {
                ...
                {
                    char[] charArrayProdData = productData.ToCharArray();
                    string tripBalance = charArrayProdData[0].ToString() + charArrayProdData[1].ToString();
                    result = Convert.ToInt32(tripBalance, 16);
                }
                return result;
            }
        ...
        public bool IsTransferTransaction(string productData)
        {
            try
            {
                ...
                {
                    char[] charArrayProdData = productData.ToCharArray();
                    string transfer = charArrayProdData[4].ToString() + charArrayProdData[5].ToString();
        ...
        public int GetNonRevenueFromProductData(string productData)
        {
            try
            {
                ...
                if (productData.Length >= 12)
                {
                    string nonRevenue = productData.Substring(0, 6);
        ...
        public int GetRevenueBalanceFromProductData(string productData)
        {
            try
            {
                ...
                if (productData.Length >= 12)
                {
                    string revenueBalance = productData.Substring(6, 6);
        ...
        public int GetHalfProductData(string productData, bool isFirstHalf)
        {
            try
            {
                ...
                if (productData.Length >= 12)
                {
                    string revenueBalance = "";
                    if (isFirstHalf)
                    {
                        revenueBalance = productData.Substring(0, 6);
                    }
                    else
                    {
                        revenueBalance = productData.Substring(6, 6);
                    }
        ...
        public DateTime? GetSmartCardExipryFromProductDate(string productData)
        {
            try
            {
                ...
                if (productData.Length >= 12)
                {
                    string hexValue = productData.Substring(8, 4);
                    string binaryValue = Convert.ToString(Convert.ToInt32(hexValue, 16), 2).PadLeft(16, '0');
                    int day = Convert.ToInt32(binaryValue.Substring(0, 5), 2);
                    int month = Convert.ToInt32(binaryValue.Substring(5, 4), 2);
                    int year = Convert.ToInt32(binaryValue.Substring(9, 7), 2) + 1991;
                    try
                    {
